...
interface Country { name: string; region: string; iso2: string; visitedYear?: number; isHome?: boolean; }
  ...
  return <div className="text-neutral-100">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_390px]">
      ...
      <aside className="rounded-md border border-slate-700/[0.4] bg-slate-700/[0.2] p-5 sm:p-6 xl:h-fit">
        ...
        <div className="divide-y divide-white/10">{selectedLocations.map((location, index) => <button type="button" onClick={() => setViewerIndex(index)} className="flex w-full items-center gap-3 py-3 text-left hover:bg-white/[0.04]" key={location.name}><IconMapPin size={18} className="shrink-0 text-neutral-400" /><div className="min-w-0 flex-1"><p className="!p-0 text-sm text-white">{location.name}</p><p className="!p-0 text-xs text-neutral-500">View photo</p></div><img src={location.imageUrl} alt="" className="h-12 w-20 rounded object-cover" /></button>)}{!selectedLocations.length && (selectedCountry?.isHome ? <div className="py-3"><p className="!p-0 text-sm text-neutral-400">Home &mdash; where most of my photography begins. My Finnish landscapes and winter scenes live in the Moments in Time gallery.</p><a href="#gallery" className="mt-4 flex w-full items-center justify-center gap-3 rounded-md border border-white/[0.2] bg-slate-700/[0.2] px-4 py-3 text-xs font-bold uppercase tracking-wider text-white hover:bg-slate-700/[0.4]">Explore Finland in the gallery <IconChevronRight size={16} /></a></div> : <p className="py-3 text-sm text-neutral-500">Visited, but no photography locations were captured on this trip.</p>)}</div>
